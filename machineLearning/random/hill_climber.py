...
class HillClimber():
    def __init__(self, As=8, n_gates=10, n_patterns=40, evolve_rate=0.5):
    ...
    def update(self, reward):
        ...
        2. Randomly generate n new patterns by adding gaussian noise to thosed kept patterns
        '''
        # step 1, 2
        ...
        for i in range((self.n_patterns-n_keep)):
            ...
            for k in range(self.n_gates):
                ...
                for j in range(self.As/2):
                    gate = self.patterns[idx, j, k]
                    noise = np.random.normal(loc=0, scale=min(abs(gate-minFemur), abs(gate-maxFemur))/2)
                    self.new_patterns[i, j, k] = self.clip(gate + noise, 350, 630)
                # Tibia
            ...
            for idx_pattern in range(self.agent.n_patterns):
                ...
                for step in range(MAX_STEP):
                    ...
                    if done:
                        ...
                        np.save('best_pattern_hc.npy', self.agent.patterns[idx_pattern])
                        break
